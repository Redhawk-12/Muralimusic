...
@app.on_message(
   filters.command(["image", "imagine", "img", "pic"] ,prefixes=["/", "!", "%", ",", "", ".", "@", "#"])
)
async def image_generatorbyshalini(client, message):
    try:
        image_name = " ".join(message.command[1:])
        if not image_name:
            await message.reply_text("ᴘʟᴇᴀsᴇ ᴇɴᴛᴇʀ ᴀɴ ɪᴍᴀɢᴇ ɴᴀᴍᴇ ᴛᴏ sᴇᴀʀᴄʜ ɪᴛ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ.")
            return
...
@app.on_message(
   filters.command(["bingimg", "bingimage", "bing"] ,prefixes=["/", "!", "%", ",", "", ".", "@", "#"])
)
async def bing_image_by_cute(client, message):
    try:
