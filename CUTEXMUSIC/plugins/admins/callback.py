...
from CUTEXMUSIC import YouTube, app
from CUTEXMUSIC.core.call import CUTE
from CUTEXMUSIC.misc import SUDOERS, db
...
from CUTEXMUSIC.utils.RanThumb import check_theme
from CUTEXMUSIC.utils.decorators.language import languageCB
from CUTEXMUSIC.utils.formatters import seconds_to_min
from CUTEXMUSIC.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from CUTEXMUSIC.utils.stream.autoclear import auto_clean
from CUTEXMUSIC.utils.thumbnails import gen_thumb
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await CUTE.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await CUTE.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await CUTE.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            theme = await check_theme(chat_id)
            ...
            try:
                await CUTE.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await CUTE.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
