...
import config
from CUTEXMUSIC import Carbon, YouTube, app
...
from CUTEXMUSIC.utils.exceptions import AssistantErr
from CUTEXMUSIC.utils.inline.play import stream_markup, queue_markup, telegram_markup
from CUTEXMUSIC.utils.inline.playlist import close_markup
from CUTEXMUSIC.utils.pastebin import CUTEbin
from CUTEXMUSIC.utils.stream.queue import put_queue, put_queue_index
from CUTEXMUSIC.utils.thumbnails import gen_thumb, gen_qthumb
...
async def stream(
    ...
    if forceplay:
        await CUTE.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await CUTE.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
